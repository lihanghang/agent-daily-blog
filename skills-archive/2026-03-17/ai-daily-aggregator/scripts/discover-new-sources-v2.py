...
def search_web(query):
    """使用 web_search 工具搜索"""
    try:
        # 调用 web_search (通过 OpenClaw)
        # 这里我们模拟结果，实际应该通过 OpenClaw API 调用
        result = subprocess.run(
            ['openclaw', 'web-search', '--query', query, '--count', '10'],
            capture_output=True,
            text=True,
            timeout=30
        )
...
def classify_relevance(url, title, snippet):
    ...
    ai_keywords = [
        ...
        "meta ai", "microsoft ai", "research paper", "arxiv", "hugging face",
    ]
