...
CHINESE_FONT_NAME = "NotoSansCJKSC-Regular"
CHINESE_FONT_BOLD = "NotoSansCJKSC-Bold"
...
def register_fonts(tokens: dict):
    """Register TTF fonts from token font paths if present, and system Chinese fonts."""
    # Register fonts from token font paths
    ...
    # Register Chinese fonts for better CJK support
    # Try system Noto Sans CJK fonts from /usr/share/fonts
    system_chinese_fonts = [
        ("NotoSansCJKSC-Regular", "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc"),
        ("NotoSansCJKSC-Bold", "/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc"),
    ]

    for name, fpath in system_chinese_fonts:
        if os.path.exists(fpath):
            try:
                # Use registerFontFamily for TTC files to try to register all faces
                pdfmetrics.registerFontFamily(f"{name}-Family", fpath)
                print(f"Registered Chinese font family: {name}-Family from {fpath}", file=sys.stderr)
            except Exception as e:
                print(f"Failed to register {name}: {e}", file=sys.stderr)
                # Try alternative: just map the name for use
                pass
    # For TTC files, we need to register the actual font face, not just the collection
    chinese_fonts = {
        "NotoSansCJKSC-Regular": "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",
        "NotoSansCJKSC-Bold": "/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc",
    }

    # Try to register Chinese fonts using registerFontFamily
    for name, fpath in chinese_fonts.items():
        if os.path.exists(fpath):
            try:
                # Register using registerFontFamily for TTC files
                # This approach tries to register all faces in the collection
                try:
                    pdfmetrics.registerFontFamily(f"{name}-Family", fpath)
                except:
                    # Fallback: try to register individual subfonts
                    pass
            except Exception:
                pass
...
def build(tokens: dict, content: list, out_path: str) -> dict:
    ...
    # Detect if content contains Chinese and use Chinese fonts
    has_chinese = False
    for item in content:
        text = item.get("text", "")
        if isinstance(text, str) and detect_chinese(text):
            has_chinese = True
            break

    # Override fonts if Chinese detected
    if has_chinese:
        # Use NotoSansCJK SC font family if registered
        # For TTC files, registerFontFamily creates names like "NotoSansCJKSC-RegularFamily-0"
        try:
            # Try different possible font family names
            chinese_body = "NotoSansCJKSC-RegularFamily-0"
            chinese_bold = "NotoSansCJKSC-BoldFamily-0"
            # Test if fonts are registered
            pdfmetrics.getFont(chinese_body)
            pdfmetrics.getFont(chinese_bold)
        except:
            # Fallback to original fonts (Helvetica) - Chinese will show as boxes
            pass
        else:
            tokens["font_body_rl"] = chinese_body
            tokens["font_body_b_rl"] = chinese_bold
            tokens["font_display_rl"] = chinese_bold
